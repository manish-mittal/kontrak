<?php
	class ACL
	{
		var $permissions = array();		//Stores the permissions for the user
		var $userID = 0;				//Stores the ID of the current user
		var $userRoles = array();		//Stores the roles of the current user
		...
		{
			$strSQL = "SELECT * FROM `user_roles` WHERE `userID` = " . intval($this->userID) . " ORDER BY `addDate` ASC";
			$data = mysqli_query($conn, $strSQL);
			$resp = array();
		...
		function getRolePerms($role)
		{
			...
			$data = mysqli_query($conn, $roleSQL);
			$perms = array();
			while($row = mysqli_fetch_assoc($data))
			{
